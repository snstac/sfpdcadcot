...
def test_adsbx_to_cot(sample_feed):
    ...
    sample_craft = sample_feed["aircraft"][0]
    cot = sfpdcadcot.adsbx_to_cot(sample_craft)
    expected = (
        ...
        b'Squawk: None Category: A1 (via sfpdcadcot@rorqual)"><UID Droid="ICAO-A9EE47" />'
        b'<contact callsign="N739UL--" /><track course="50.1" speed="40.641076" /><remarks>N739UL-- ICAO: '
        b"A9EE47 REG:  Flight: N739UL Type:  Squawk: None Category: A1 (via sfpdcadcot@rorqual)</remarks>"
        b"</detail></event>"
    )
...
def test_adsbx_to_cot_xml_with_known_craft(sample_feed, sample_known_craft):
    sample_craft = sample_feed["aircraft"][0]
    known_craft_key = "REG"
    ...
    cot = sfpdcadcot.functions.adsbx_to_cot_xml(sample_craft, known_craft=known_craft)
    print(ET.tostring(cot))
